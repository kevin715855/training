...
int main()
{
  ...
  ll curT = t[1], curA = a[1];
  FOR(i, 2, n) {
    //cout << db(curT) << ' ' << db(curA) << endl;
    ll nxT = (curT/t[i]) + (curT%t[i] != 0);
    ll nxA = (curA/a[i]) + (curA%a[i] != 0);
    //cout << db(max(nxT, nxA)) << endl;
    curT = t[i]* max(nxT, nxA);
    curA = a[i]* max(nxT, nxA);
  }
  cout << curT + curA;
}
